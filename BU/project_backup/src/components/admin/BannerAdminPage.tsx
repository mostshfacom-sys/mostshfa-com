...
interface BannerAdminPageProps {
  pageKey: string;
  heading: string;
  description: string;
  defaultTitle: string;
...
export default function BannerAdminPage({
  ...
  useEffect(() => {
    const fetchBanner = async () => {
      try {
        const [pageRes, masterRes] = await Promise.all([
          fetch(`/api/admin/banner?pageKey=${pageKey}`),
        ...
        if (pageRes.ok) {
          ...
          if (data?.banner) {
            ...
            setBanner((prev) => ({
              ...
              subtitle: normalized.subtitle || defaultSubtitle,
              overlayColor: normalized.overlayColor || defaultOverlayColor,
              overlayOpacity:
                typeof normalized.overlayOpacity === 'number'
                  ? normalized.overlayOpacity
    ...
    fetchBanner();
  }, [defaultOverlayColor, defaultOverlayOpacity, defaultSubtitle, defaultTitle, pageKey]);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const baseName = (fileNameBase || pageKey).trim();
  ...
  const previewOverlayColor = banner.overlayColor?.trim() || defaultOverlayColor;
  const previewOverlayOpacity =
    typeof banner.overlayOpacity === 'number' ? banner.overlayOpacity : defaultOverlayOpacity;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input
                    type="color"
                    value={banner.overlayColor || defaultOverlayColor}
                  ...
                  <input
                    ...
                    placeholder={defaultOverlayColor}
                    dir="ltr"
                  />
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    min={0}
                    max={100}
                    value={banner.overlayOpacity ?? defaultOverlayOpacity}
                    onChange={(e) =>
                  ...
                  <span className="min-w-[48px] text-sm font-semibold text-gray-700">
                    {banner.overlayOpacity ?? defaultOverlayOpacity}%
                  </span>
                </div>
              </div>
            </div>
