...
export function UniversalSmartHeaderCompact({
  title,
  subtitle,
  counters = [],
  searchValue = '',
  ...
  const resultsLabelText = resultsLabel?.trim() || 'نتيجة';
  const resolvedBadgeLabel = badgeLabel ?? title;
  const resolvedBreadcrumbLabel = breadcrumbLabel ?? title;
  ...
  return (
    <header className={`hospitals-header relative text-white overflow-hidden ${className}`}>
      ...
      >
        <div className="max-w-7xl mx-auto space-y-8 sm:space-y-12">
          ...
          >
            ...
            <span>الرئيسية</span>
            <span>/</span>
            <span className="text-white font-medium">{resolvedBreadcrumbLabel}</span>
          </div>
        ...
        >
          <div className="flex flex-col gap-3 sm:gap-5">
            ...
            >
              ...
              <div
                className={`hidden sm:flex flex-wrap items-stretch gap-2 sm:gap-3 w-full ${
                  hasCounters ? 'lg:w-auto lg:justify-end' : 'justify-center'
                }`}
              >
                {showMapButton && (
                  <motion.button
            ...
            <div className="flex flex-nowrap justify-center gap-2 sm:hidden overflow-x-auto">
              {shouldShowQuickFiltersToggle && (
                <motion.button
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.97 }}
                  onClick={handleQuickFiltersToggle}
                  className={`flex flex-col items-center justify-center gap-1 px-2 py-2 bg-gradient-to-r backdrop-blur-md border-2 rounded-lg font-medium transition-all shadow-lg whitespace-nowrap cursor-pointer flex-none text-[10px] ${
                    quickFiltersExpanded
                      ? 'from-purple-500/30 to-pink-500/30 border-purple-400/50 hover:shadow-purple-500/20'
                      : 'from-purple-500/20 to-pink-500/20 border-purple-400/30 hover:from-purple-500/30 hover:to-pink-500/30 hover:border-purple-400/50 hover:shadow-purple-500/20'
                  }`}
              ...
              {showMapButton && (
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  onClick={() => setIsMapOpen(true)}
                  className="flex flex-col items-center justify-center gap-1 px-2 py-2 text-[10px] bg-gradient-to-r from-cyan-500/40 to-blue-500/40 hover:from-cyan-500/50 hover:to-blue-500/50 backdrop-blur-md border-2 border-cyan-400/50 hover:border-cyan-400/70 text-white rounded-lg font-medium transition-all shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/40 whitespace-nowrap cursor-pointer flex-none"
                  type="button"
                  title={mapButtonLabel}
                >
                  <MapIcon className="w-4 h-4 text-white" />
                  <span className="text-[10px] leading-tight">{mapButtonLabel}</span>
                </motion.button>
              )}

              {shouldShowResultsCount && (
                <div className="flex flex-col items-center justify-center gap-0.5 px-2 py-2 bg-white/15 backdrop-blur-md border-2 border-white/30 rounded-lg flex-none">
                  <span className="font-bold text-xs tabular-nums">
                    {resultsTotal.toLocaleString('ar-EG')}
                  </span>
                  <span className="text-[10px] text-white/80">{resultsLabelText}</span>
                </div>
              )}
              ...
              {showViewToggle && onViewModeChange && (
                <button
                  ...
                  className="flex flex-col items-center justify-center gap-1 px-2 py-2 bg-white/15 backdrop-blur-md border-2 border-white/30 rounded-lg text-white shadow-md text-[10px] flex-none"
            ...
            {hasQuickFilters && (
              <div className="relative min-h-[64px] sm:min-h-[64px]">
                <motion.div
                  ...
                  className={`flex flex-wrap sm:flex-nowrap items-center justify-center sm:justify-center gap-2 overflow-x-hidden sm:overflow-x-auto overflow-y-visible py-2 px-1 sm:px-0 ${
        ...
        {isMapOpen && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-6xl bg-white rounded-2xl overflow-hidden shadow-2xl flex flex-col my-6 max-h-[calc(100vh-3rem)]"
              onClick={(event) => event.stopPropagation()}
            >
              <div className="flex flex-wrap items-start justify-between gap-2 px-4 py-3 bg-gradient-to-r from-cyan-600 to-blue-600 text-white">
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg sm:text-xl font-bold text-white">{mapTitle}</h3>
                  {mapSubtitle && <p className="text-sm text-white/80">{mapSubtitle}</p>}
                </div>
                <button
                  type="button"
                  onClick={() => setIsMapOpen(false)}
      ...
      <div className="absolute bottom-0 left-0 right-0 pointer-events-none">
        <svg
          viewBox="0 0 1440 80"
          className="w-full h-7 sm:h-9 text-gray-50 dark:text-slate-950"
...
export default UniversalSmartHeaderCompact;
