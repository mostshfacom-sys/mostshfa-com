...
export const metadata: Metadata = {
  title: 'دليل الأدوية السابق | مستشفى.كوم',
  description:
    'النسخة السابقة من صفحة دليل الأدوية، متاحة مؤقتًا للمراجعة الداخلية بعد نقل الصفحة الأساسية إلى التجربة الجديدة.',
  alternates: {
    canonical: '/old-drugs',
  },
  openGraph: {
    title: 'دليل الأدوية السابق | مستشفى.كوم',
    description:
      'نسخة أرشيفية من صفحة دليل الأدوية السابقة.',
  },
  robots: {
    index: false,
    follow: false,
...
export default async function DrugsPage({ searchParams }: PageProps) {
  ...
  const buildLink = (updates: Record<string, string | undefined>) => {
    ...
    return query ? `/old-drugs?${query}` : '/old-drugs';
  };
        ...
        <div className="container-custom pb-10">
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {data.categories.slice(0, 12).map((cat) => (
                  <Link
                    key={cat.id}
                    href={`/old-drugs?category=${cat.id}`}
          ...
          {data.drugs.length === 0 && (
            <div className="text-center py-12">
              ...
              <Link href="/old-drugs" className="text-primary-600 dark:text-primary-300 hover:underline mt-2 inline-block">
                عرض جميع الأدوية
              </Link>
            </div>
          )}
          ...
          {data.totalPages > 1 && (
            <div className="flex justify-center gap-2 mt-8 flex-wrap">
              {data.page > 1 && (
                <Link
                  href={`/old-drugs?page=${data.page - 1}${searchParams.search ? `&search=${searchParams.search}` : ''}${searchParams.category ? `&category=${searchParams.category}` : ''}`}
              ...
              {Array.from({ length: Math.min(5, data.totalPages) }, (_, i) => {
                ...
                return (
                  <Link
                    key={pageNum}
                    href={`/old-drugs?page=${pageNum}${searchParams.search ? `&search=${searchParams.search}` : ''}${searchParams.category ? `&category=${searchParams.category}` : ''}`}
